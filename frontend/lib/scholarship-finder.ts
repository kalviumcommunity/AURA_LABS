...
import type { Scholarship, EntranceExam } from "@/types/scholarship"
...
export class ScholarshipFinder {
  private scholarships: Scholarship[] = scholarshipsData
  private exams: EntranceExam[] = entranceExamsData

  findRelevantScholarships(userData: QuestionnaireData): Scholarship[] {
  ...
  findRelevantExams(userData: QuestionnaireData): EntranceExam[] {
    return this.exams
  ...
  private isScholarshipEligible(scholarship: Scholarship, userData: QuestionnaireData): boolean {
  ...
  private isExamRelevant(exam: EntranceExam, userData: QuestionnaireData): boolean {
    // Check education level
  ...
  private calculateScholarshipScore(scholarship: Scholarship, userData: QuestionnaireData): number {
    let score = 0
  ...
  private calculateExamRelevance(exam: EntranceExam, userData: QuestionnaireData): number {
    let score = 0
  ...
  getExamById(examId: string): EntranceExam | undefined {
    return this.exams.find((exam) => exam.id === examId)
  }

  getScholarshipById(scholarshipId: string): Scholarship | undefined {
