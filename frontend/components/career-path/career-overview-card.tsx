...
export function CareerOverviewCard({ careerPath }: CareerOverviewCardProps) {
  const getOutlookColor = (outlook: CareerPath["jobOutlook"]) => {
    switch (outlook) {
      case "excellent":
        return "bg-green-500"
      case "good":
  ...
  const getOutlookText = (outlook: CareerPath["jobOutlook"]) => {
    switch (outlook) {
      case "excellent":
        return "Excellent"
      case "good":
